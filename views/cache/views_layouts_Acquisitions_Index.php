...
<nav aria-label="breadcrumb" class="mt-2">
	<ol class="breadcrumb">
		<li class="breadcrumb-item">
			<a class="display-6" style="text-decoration: none;" href="<?php echo route('/acquisitions') ?>">Acquisitions</a>
		</li>
	</ol>
</nav>
...
<script src="<?php echo asset('scripts/acquisition.js') ?>"></script>
<script>
	$(document).ready(function() {
		$("#createEquipment").click(function(e) {
			...
			$.ajax({
				url: "<?php echo route('/acquisitions/create'); ?>",
				method: "GET",
				dataType: "JSON",
				success: function(response) {
					if (response.status == "success") {
						let url = '<?php echo route("/acquisitions/save/") ?>' + "/" + response.id;
	...
	function deleteAcquisitions(obj){
			...
			if(conf){
				$.ajax({
					url: "<?php echo route('/acquisitions/delete/'); ?>"+ "/" + obj.id,
					method: "GET",
					dataType: "JSON",
					success: function(response) {
						if (response.status == "success") {
							alert(response.message)
							let url = '<?php echo route("/acquisitions") ?>';
							$(location).attr("href", url);
						}
					},
					error:function(obj,error){

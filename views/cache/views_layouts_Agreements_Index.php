...
<nav aria-label="breadcrumb" class="mt-2">
	<ol class="breadcrumb">
		<li class="breadcrumb-item">
			<a class="display-6" style="text-decoration: none;" href="<?php echo route('/agreements') ?>">Agreements</a>
		</li>
	</ol>
</nav>
...
<script src="<?php echo asset('scripts/agreement.js') ?>"></script>
<script>
	$(document).ready(function() {
		$("#createAgreement").click(function(e) {
			...
			$.ajax({
				url: "<?php echo route('/agreements/create'); ?>",
				method: "GET",
				dataType: "JSON",
				success: function(response) {
					if (response.status == "success") {
						let url = '<?php echo route("/agreements/save/") ?>' + "/" + response.id;
	...
	function deleteAgreement(obj){
			...
			if(conf){
				$.ajax({
					url: "<?php echo route('/agreements/delete/'); ?>"+ "/" + obj.id,
					method: "GET",
					dataType: "JSON",
					success: function(response) {
						if (response.status == "success") {
							alert(response.message)
							let url = '<?php echo route("/agreements") ?>';
							$(location).attr("href", url);
						}
					},
					error:function(obj,error){

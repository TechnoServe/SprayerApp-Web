...
<nav aria-label="breadcrumb" class="mt-2">
	<ol class="breadcrumb">
		<li class="breadcrumb-item">
			<a class="display-6" style="text-decoration: none;" href="<?php echo route('/applications') ?>">Applications</a>
		</li>
	</ol>
</nav>
...
<script src="<?php echo asset('scripts/application.js') ?>"></script>
<script>
	$(document).ready(function() {
		$("#createApplication").click(function(e) {
			...
			$.ajax({
				url: "<?php echo route('/applications/create'); ?>",
				method: "GET",
				dataType: "JSON",
				success: function(response) {
					if (response.status == "success") {
						let url = '<?php echo route("/applications/save/") ?>' + "/" + response.id;
	...
	function deleteApplication(obj){
			...
			if(conf){
				$.ajax({
					url: "<?php echo route('/applications/delete/'); ?>"+ "/" + obj.id,
					method: "GET",
					dataType: "JSON",
					success: function(response) {
						if (response.status == "success") {
							alert(response.message)
							let url = '<?php echo route("/applications") ?>';
							$(location).attr("href", url);
						}
					},
					error:function(obj,error){

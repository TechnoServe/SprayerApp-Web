...
<body class="text-center">
  <main class="form-signin">
    ...
    <form name="formSignIn" id="formSignIn">
      <img class="mb-4" src=<?php echo asset('technoserve-banner-transparent.png') ?> alt="Technoserve Logo" width="300" >
...
<!-- Close the Modal to register a seller -->
  ...
  <script type="text/javascript">
    ...
    document.querySelector("#btnLogin").addEventListener("click", function(event){
      ...
      let url = "<?php echo route('/authanticate') ?>";
    ...
    document.querySelector("#subtmitRequest").addEventListener("click", function(){
      ...
      let formData = new FormData(form);
      let url = "<?php echo route('/request/seller/profile') ?>";
      ...
      .then(resp => resp.json())
        .then(resp => {
          ...
          if(resp.status == "success"){
            form.reset();
            location.href = "<?php echo route('/login') ?>";
          }
        })
      .catch(error => {
          buildAlert(error.status, error.message);

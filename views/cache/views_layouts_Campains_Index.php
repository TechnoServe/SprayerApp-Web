...
<nav aria-label="breadcrumb" class="mt-2">
	<ol class="breadcrumb">
		<li class="breadcrumb-item">
			<a class="display-6" style="text-decoration: none;" href="<?php echo route('/campaigns') ?>">Campaigns</a>
		</li>
	</ol>
</nav>
...
<div class="table-responsive table-responsive-lg">
	<table id="result" class="table table-striped table-bordered table-condensed table-sm dt-responsive nowrap" style="width: 100%;">
		...
		<tfoot>
			<tr>
				<th class="filterFooter">Designation</th>
				<th class="filterFooter">Starts</th>
				<th class="filterFooter">Finishes</th>
				<th class="filterFooter">Status</th>
				<th class="filterFooter">Campaign Status</th>
				<th>Actions</th>
			</tr>
		</tfoot>
	</table>
</div>
<script src="<?php echo asset('scripts/campaign.js') ?>"></script>
<script>
	$(document).ready(function() {
		$("#createCampain").click(function(e) {
			...
			$.ajax({
				url: "<?php echo route('/campaigns/create'); ?>",
				method: "GET",
				dataType: "JSON",
				success: function(response) {
					if (response.status == "success") {
						let url = '<?php echo route("/campaigns/save/") ?>' + "/" + response.id;

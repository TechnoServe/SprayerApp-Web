...
<nav aria-label="breadcrumb" class="mt-2">
	<ol class="breadcrumb">
		<li class="breadcrumb-item">
			<a class="display-6" style="text-decoration: none;" href="<?php echo route('/dropdowns') ?>">Dropdowns</a>
		</li>
	</ol>
</nav>
<input type="hidden" id="url" data-url="<?php echo route('') ?>">
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3">
	<a class="btn btn-primary btn-lg" href="#" id="createDropdown">Create dropdown</a>
</div>
...
<script src="<?php echo asset('scripts/dropdown.js') ?>"></script>
<script>
	$(document).ready(function() {
		$("#createDropdown").click(function(e) {
			e.preventDefault();

			let _this = $(this);
			let oldText = _this.text();
			$.ajax({
				url: "<?php echo route('/dropdowns/create'); ?>",
				method: "GET",
				dataType: "JSON",
				success: function(response) {
					if (response.status == "success") {
						let url = '<?php echo route("/dropdowns/save/") ?>' + "/" + response.id;
						$(location).attr("href", url);
					}
				},
				error : function(error, status, message){
					console.log(`Error calling the form to create dropdowns: `, status, message, error);

			...
			<nav id="sidebarMenu" class="col-md-3 col-lg-2 d-md-block bg-light sidebar collapse">
				<div class="position-sticky pt-3 mt-5">
					...
					<ul class="nav flex-column mb-2">
						...
						<li class="nav-item">
							<a class="nav-link" href="#finances_section" data-bs-toggle="collapse">Finances</a>
							<ul class="nav-item collapse" id="finances_section">
								<li data-route="<?php echo route('/payments') ?>" class="nav-item pointer">Payments</li>
								<li data-route="<?php echo route('/incomes') ?>" class="nav-item pointer">Expenses / Incomes</li>
								<li data-route="<?php echo route('/agreements') ?>" class="nav-item pointer">Agreements </li>
							</ul>
						</li>

						<li class="nav-item">
							<a class="nav-link" href="#settings_section" data-bs-toggle="collapse">Settings</a>
							<ul class="nav-item collapse" id="settings_section">
								<li data-route="<?php echo route('/dropdowns') ?>" class="nav-item pointer">Dropdowns</li>
								<li data-route="<?php echo route('/faqs') ?>" class="nav-item pointer">FAQ</li>
								<li data-route="<?php echo route('/logs') ?>" class="nav-item pointer">Logs</li>
							</ul>
						</li>
					</ul>
				</div>
...
<nav aria-label="breadcrumb" class="mt-2">
	<ol class="breadcrumb">
		<li class="breadcrumb-item"><a class="display-6" style="text-decoration: none;" href="<?php echo route('/faqs') ?>">Dropdowns</a></li>
...
<form name="saveDropdown" id="saveDropdown" class="">
	<div class="row g-3">
		<input type="hidden" id="id" value="<?php echo $faq ['id'] ?? '' ?>">
		<div class="col-sm-12">
			<label for="name" class="form-label">Name/Key <span class="text-danger">*</span></label>
			<input type="text" class="form-control" name="name" id="name" value="<?php echo $faq ['key'] ?? '' ?>">
		</div>
		<div class="col-sm-12">
			<label for="value" class="form-label">Description  <span class="text-danger">*</span></label>
			<textarea class="form-control" name="value" id="value"><?php echo $faq ['value'] ?? '' ?></textarea>
		...
		<div class="col-sm-12">
			...
			<select class="form-select" id="type" name="type">
				<?php foreach([ 'String', 'Integer', 'Number', 'Boolean'] as $type): ?>
				<option value="<?php echo $type ?>" <?php echo $faq ['type'] == $type ? 'selected' : '' ?>><?php echo $type ?></option>
				<?php endforeach; ?>
			</select>
		</div>
		<div class="col-sm-12">
	...
	$(document).ready(function() {
		...
		$("#btnSaveDropdown").click(function(e) {
			try{
				...
				$.ajax({
					url: "<?php echo route('/faqs/save/'.$faq ['id']) ?>",
					method: "POST",
					dataType: "JSON",
					data: formData,

...
<nav aria-label="breadcrumb" class="mt-2">
	<ol class="breadcrumb">
		<li class="breadcrumb-item">
			<a class="display-6" style="text-decoration: none;" href="<?php echo route('/equipments') ?>">Equipments</a>
		</li>
	</ol>
</nav>
...
<script src="<?php echo asset('scripts/equipment.js') ?>"></script>
<script>
	$(document).ready(function() {
		$("#createEquipment").click(function(e) {
			...
			$.ajax({
				url: "<?php echo route('/equipments/create'); ?>",
				method: "GET",
				dataType: "JSON",
				success: function(response) {
					if (response.status == "success") {
						let url = '<?php echo route("/equipments/save/") ?>' + "/" + response.id;
	...
	function deleteEquipments(obj){
			...
			if(conf){
				$.ajax({
					url: "<?php echo route('/equipments/delete/'); ?>"+ "/" + obj.id,
					method: "GET",
					dataType: "JSON",
					success: function(response) {
						if (response.status == "success") {
							alert(response.message)
							let url = '<?php echo route("/equipments") ?>';
							$(location).attr("href", url);
						}
					},
					error:function(obj,error){

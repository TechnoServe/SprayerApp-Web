...
<nav aria-label="breadcrumb" class="mt-2">
	<ol class="breadcrumb">
		<li class="breadcrumb-item">
			<a class="display-6" style="text-decoration: none;" href="<?php echo route('/faqs') ?>">Faqs</a>
		</li>
	</ol>
</nav>
...
<script src="<?php echo asset('scripts/faq.js') ?>"></script>
<script>
	$(document).ready(function() {
		$("#createFaq").click(function(e) {
			...
			$.ajax({
				url: "<?php echo route('/faqs/create'); ?>",
				method: "GET",
				dataType: "JSON",
				success: function(response) {
					if (response.status == "success") {
						let url = '<?php echo route("/faqs/save/") ?>' + "/" + response.id;

...
<nav aria-label="breadcrumb" class="mt-2">
	<ol class="breadcrumb">
		<li class="breadcrumb-item">
			<a class="display-6" style="text-decoration: none;" href="<?php echo route('/farmers') ?>">Farmers</a>
		</li>
	</ol>
</nav>
...
<script src="<?php echo asset('scripts/farmer.js') ?>"></script>
<script type="text/javascript">
	$(document).ready(function() {
		$("#createFarmer").click(function(e) {
			...
			$.ajax({
				url: "<?php echo route('/farmers/create'); ?>",
				method: "GET",
				dataType: "JSON",
				success: function(response) {
					if (response.status == "success") {
						let url = '<?php echo route("/farmers/save/") ?>' + "/" + response.id;
	...
	function deleteFarmer(obj){
			...
			if(conf){
				$.ajax({
					url: "<?php echo route('/farmers/delete/'); ?>"+ "/" + obj.id,
					method: "GET",
					dataType: "JSON",
					success: function(response) {
						if (response.status == "success") {
							alert(response.message)
							let url = '<?php echo route("/farmers") ?>';
							$(location).attr("href", url);
						}
					},
					error:function(obj,error){

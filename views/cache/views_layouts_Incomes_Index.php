...
<nav aria-label="breadcrumb" class="mt-2">
	<ol class="breadcrumb">
		<li class="breadcrumb-item">
			<a class="display-6" style="text-decoration: none;" href="<?php echo route('/incomes') ?>">Expenses or Incomes</a>
		</li>
	</ol>
</nav>
...
<script src="<?php echo asset('scripts/income.js') ?>"></script>
<script>
	$(document).ready(function() {
		$("#createEquipment").click(function(e) {
			...
			$.ajax({
				url: "<?php echo route('/incomes/create'); ?>",
				method: "GET",
				dataType: "JSON",
				success: function(response) {
					if (response.status == "success") {
						let url = '<?php echo route("/incomes/save/") ?>' + "/" + response.id;
	...
	function deleteIncomes(obj){
			...
			if(conf){
				$.ajax({
					url: "<?php echo route('/incomes/delete/'); ?>"+ "/" + obj.id,
					method: "GET",
					dataType: "JSON",
					success: function(response) {
						if (response.status == "success") {
							alert(response.message)
							let url = '<?php echo route("/incomes") ?>';
							$(location).attr("href", url);
						}
					},
					error:function(obj,error){

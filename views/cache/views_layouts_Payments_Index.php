...
<nav aria-label="breadcrumb" class="mt-2">
	<ol class="breadcrumb">
		<li class="breadcrumb-item">
			<a class="display-6" style="text-decoration: none;" href="<?php echo route('/payments') ?>">Payments</a>
		</li>
	</ol>
</nav>
...
<script src="<?php echo asset('scripts/payment.js') ?>"></script>
<script>
	$(document).ready(function() {
		$("#createPayment").click(function(e) {
			...
			$.ajax({
				url: "<?php echo route('/payments/create'); ?>",
				method: "GET",
				dataType: "JSON",
				success: function(response) {
					if (response.status == "success") {
						let url = '<?php echo route("/payments/save/") ?>' + "/" + response.id;
	...
	function deletePayment(obj){
			...
			if(conf){
				$.ajax({
					url: "<?php echo route('/payments/delete/'); ?>"+ "/" + obj.id,
					method: "GET",
					dataType: "JSON",
					success: function(response) {
						if (response.status == "success") {
							alert(response.message)
							let url = '<?php echo route("/payments") ?>';
							$(location).attr("href", url);
						}
					},
					error:function(obj,error){

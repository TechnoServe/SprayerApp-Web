...
<nav aria-label="breadcrumb" class="mt-2">
	<ol class="breadcrumb">
		<li class="breadcrumb-item">
			<a class="display-6" style="text-decoration: none;" href="<?php echo route('/profiles') ?>">Profiles</a>
		</li>
	</ol>
</nav>
...
<script src="<?php echo asset('scripts/profile.js') ?>"></script>
<script>
	$(document).ready(function() {
		$("#createProfile").click(function(e) {
			...
			$.ajax({
				url: "<?php echo route('/profiles/create'); ?>",
				method: "GET",
				dataType: "JSON",
				success: function(response) {
					if (response.status == "success") {
						let url = '<?php echo route("/profiles/save/") ?>' + "/" + response.id;

...
<nav aria-label="breadcrumb" class="mt-2">
	<ol class="breadcrumb">
		<li class="breadcrumb-item">
			<a class="display-6" style="text-decoration: none;" href="<?php echo route('/requests') ?>">Requests</a>
		</li>
	</ol>
</nav>
...
<script src="<?php echo asset('scripts/request.js') ?>"></script>
<script>
	function updateApproved(id, status){
		...
		let url = "<?php echo route('/requests/:id/approve') ?>";
		    ...
		    fetch(url, {
              ...
              .then(resp => {
                ...
                if(resp.status == "success"){
                 buildAlert(resp.status, resp.message);
                  location.href = "<?php echo route('/requests') ?>";
                }
              })
              .catch(error => {

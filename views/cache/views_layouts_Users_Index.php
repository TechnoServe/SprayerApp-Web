...
<nav aria-label="breadcrumb" class="mt-2">
	<ol class="breadcrumb">
		<li class="breadcrumb-item">
			<a class="display-6" style="text-decoration: none;" href="<?php echo route('/users') ?>">Users</a>
		</li>
	</ol>
</nav>
...
<script src="<?php echo asset('scripts/user.js') ?>"></script>
<script>
	$(document).ready(function() {
		$("#createUser").click(function(e) {
			...
			$.ajax({
				url: "<?php echo route('/users/create'); ?>",
				method: "GET",
				dataType: "JSON",
				success: function(response) {
					if (response.status == "success") {
						let url = '<?php echo route("/users/save/") ?>' + "/" + response.id;

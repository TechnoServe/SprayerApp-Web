...
<nav aria-label="breadcrumb" class="mt-2">
	<ol class="breadcrumb">
		<li class="breadcrumb-item">
			<a class="display-6" style="text-decoration: none;" href="{{ route('/campaigns') }}">Campaigns</a>
		</li>
	</ol>
</nav>
...
<script src="{{ asset('scripts/campaign.js') }}"></script>
<script>
			...
			$.ajax({
				...
				success: function(response) {
					if (response.status == "success") {
						let url = '{{ route("/campaigns/save/") }}' + "/" + response.id;
